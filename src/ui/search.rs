    ...
    pub fn build(state: Rc<RefCell<AppState>>, nav: adw::NavigationView) -> adw::NavigationPage {
        ...
        search_entry.connect_changed(clone!(
            ...
            move |entry| {
                ...
                if let Some(id) = pending.borrow_mut().take() { id.remove(); }
                ...
                MainContext::default().spawn_local(async move {
                    match geocube::get_current_location().await {
                        Some(coords) => match nominatim::reverse(coords).await {
                            Ok(Some(place)) => match sync::add_location(place, &st.borrow().db).await {
                                Ok(_) => { sl.set_text("Location added!"); sl.set_visible(true); }
...
fn build_result_row(place: &Place, state: Rc<RefCell<AppState>>, popover: gtk::Popover) -> gtk::ListBoxRow {
    let hbox = gtk::Box::builder().orientation(gtk::Orientation::Horizontal).spacing(8)
        .margin_start(8).margin_end(8).margin_top(6).margin_bottom(6).build();

    let name_label = gtk::Label::builder().label(&place.name).halign(gtk::Align::Start).hexpand(true).build();
    let sub = place.state.as_deref().map(|s| format!("{}, {}", s, place.country_code))
        .unwrap_or_else(|| place.country_code.clone());
    let sub_label = gtk::Label::builder().label(&sub).halign(gtk::Align::End).css_classes(["dim-label"]).build();
    ...
    row.connect_activate(move |_| {
        ...
        let pop = popover.clone();
        MainContext::default().spawn_local(async move {
            match sync::add_location(place2, &st.borrow().db).await {
